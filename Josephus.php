		...
		foreach($prisoners as $thisPrisoner=>$dead){
			if(!$dead){										//so yeah...if not dead...
				if($deadpool==$k){							//if thier time is up in the deadpool...
					$order++;
					$prisoners[$thisPrisoner]=((($n-$m)>($order)?$order:(($n)==$order?'Call me *Titus Flavius* Josephus':'Joe\'s friend '.(($order)-($n-$m-1)))));//set the deadpool value or enumerate as survivor
